...
/// Pick an active EMI plan for a repayment transaction.
Future<String?> showPickEmiPlanSheet(BuildContext context) {
  return showModalBottomSheet<String>(
    context: context,
    isScrollControlled: true,
    shape: const RoundedRectangleBorder(
    ...
    builder: (_) => const _PickEmiPlanSheet(),
  );
}

class _PickEmiPlanSheet extends ConsumerWidget {
  const _PickEmiPlanSheet();

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final plans = ref.watch(activeEmiPlansProvider);
    ...
    return SafeArea(
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            const SizedBox(height: 12),
            Text('Link to EMI', style: Theme.of(context).textTheme.titleMedium),
            const SizedBox(height: 8),
            plans.when(
              ...
              error: (e, _) => Padding(
                ...
                child: Text('Could not load EMIs: $e'),
              ),
              data: (list) {
                if (list.isEmpty) {
                  return const Padding(
                    padding: EdgeInsets.all(24),
                    child: Text('No active EMIs. Mark a purchase as EMI first.'),
                  );
                }
                return Flexible(
                    ...
                    itemBuilder: (context, i) {
                      ...
                      return ListTile(
                        ...
                        subtitle: Text(
                          ...
                          '${plan.pendingInstallments} left · '
                          '${currency.format(plan.principalAmount)}',
                        ),
                        trailing: Text(
                          currency.format(plan.expectedMonthly),
...
typedef SplitSettlementLink = ({String billId, String participantId});

/// Pick an active split bill and unpaid participant for a settlement.
Future<SplitSettlementLink?> showLinkSplitSettlementSheet(BuildContext context) {
  return showModalBottomSheet<SplitSettlementLink>(
    context: context,
    isScrollControlled: true,
    shape: const RoundedRectangleBorder(
      borderRadius: BorderRadius.vertical(top: Radius.circular(20)),
    ),
    builder: (_) => const _LinkSplitSettlementSheet(),
  );
}

class _LinkSplitSettlementSheet extends ConsumerStatefulWidget {
  const _LinkSplitSettlementSheet();

  @override
  ConsumerState<_LinkSplitSettlementSheet> createState() => _LinkSplitSettlementSheetState();
}

class _LinkSplitSettlementSheetState extends ConsumerState<_LinkSplitSettlementSheet> {
  SplitBill? _selectedBill;

  @override
  Widget build(BuildContext context) {
    final bills = ref.watch(activeSplitBillsProvider);
    final currency = NumberFormat.currency(locale: 'en_IN', symbol: '₹', decimalDigits: 0);
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            const SizedBox(height: 12),
            Text(
              _selectedBill == null ? 'Link split settlement' : 'Who paid?',
              style: Theme.of(context).textTheme.titleMedium,
            ),
            const SizedBox(height: 8),
            if (_selectedBill == null)
              bills.when(
                ...
                data: (list) {
                  if (list.isEmpty) {
                    return const Padding(
                      padding: EdgeInsets.all(24),
                      child: Text('No active splits. Mark an expense as Split first.'),
                    );
                  }
                  return Flexible(
                    child: ListView.builder(
                      ...
                      itemBuilder: (context, i) {
                        ...
                        return ListTile(
                          ...
                          onTap: () async {
                            final api = ref.read(apiProvider);
                            final detail = await api.get('/split-bills/${bill.id}')
                                as Map<String, dynamic>;
                            if (mounted) {
                              setState(() => _selectedBill = SplitBill.fromJson(detail));
                            }
                          },
                        );
                      },
              ...
              ListTile(
                title: Text(_selectedBill!.merchant),
                subtitle: Text(currency.format(_selectedBill!.totalAmount)),
                trailing: TextButton(
                  onPressed: () => setState(() => _selectedBill = null),
                  child: const Text('Back'),
                ),
              ),
              ..._selectedBill!.participants.where((p) => !p.isPaid).map(
                (p) => ListTile(
                  title: Text(p.name),
                  subtitle: Text(currency.format(p.shareAmount)),
                  onTap: () => Navigator.pop(
                    context,
                    (billId: _selectedBill!.id, participantId: p.id),
                  ),
                ),
              ),
              if (_selectedBill!.participants.every((p) => p.isPaid))
                const Padding(
                  padding: EdgeInsets.all(24),
                  child: Text('Everyone has paid for this split.'),
                ),
            ],
            const SizedBox(height: 16),
          ],
